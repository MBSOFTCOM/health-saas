...
import { dbName, tbScreenElectrocardiogram, tbScreenSum } from '@/utils/sqlite';
import {
	screenType,
	getLabelByValue,
	genderMap,
...
export default {
	...
	methods: {
		...
		// 搜索
		search() {
			SynchronizeApi.getElectrocardiogramCount(this.queryParams.screenId, this.queryParams.screenPoint).then(
				(res) => {
					if (res[0].num > 0) {
						...
						if (res.confirm) {
							if (self.pageData.length == 0) {
								...
								SynchronizeApi.getElectrocardiogramData(
									...
									// 上传
									SynchronizeApi.updateTableData6(self.SyncData).then((res) => {
										// 上传心电组图片
										SynchronizeApi.uploadOfflineImage(5);
										if (res.data) {
											uni.showToast({
												title: '上传成功',
												mask: true,
