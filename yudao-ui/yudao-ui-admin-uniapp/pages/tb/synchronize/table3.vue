...
export default {
	...
	methods: {
		...
		search() {
			...
			SynchronizeApi.getTableData3(this.queryParams).then((resp) => {
										...
										SynchronizeApi.getTableData3(self.queryParams).then((resp) => {
											...
											self.SyncData.forEach((item) => {
												...
												SynchronizeApi.selectTableIdByIndex(
													...
													// 处理null值
													if (
														item.injectionAgency == null ||
														item.injectionAgency == 'null'
													) {
														item.injectionAgency = '';
													}
													if (
													...
													if (res.length > 0) {
														...
														SynchronizeApi.updateTable(tbScreenPpd, updateData, id);
													} else {
														getById(dbName, tbScreenPpd, item.id).then((res) => {
															if (res.length > 0) {
																SynchronizeApi.selectMaxId(tbScreenPpd).then((resp) => {
																	if (resp.length > 0) {
																...
																delete addData.name;
																dbUtils.addTabItem(dbName, tbScreenPpd, addData);
															}
														});
													}
												});
								...
								if (res.confirm) {
									...
									self.SyncData.forEach((item) => {
										...
										SynchronizeApi.selectTableIdByIndex(
											...
											// 处理null值
											if (item.injectionAgency == null || item.injectionAgency == 'null') {
												item.injectionAgency = '';
											}
													...
													if (res.length > 0) {
														SynchronizeApi.selectMaxId(tbScreenPpd).then((resp) => {
															if (resp.length > 0) {
																...
																delete newUpdateData.name;
																// 执行插入操作
																dbUtils.addTabItem(dbName, tbScreenPpd, newUpdateData);
															}
														});
													} else {
														let addData = {
