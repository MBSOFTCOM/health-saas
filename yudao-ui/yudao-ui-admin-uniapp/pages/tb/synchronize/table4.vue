		...
		<view class="uni-container">
			...
			>
				...
				<uni-tr v-for="(item, index) in pageData" :key="index">
					...
					<uni-td class="uni-td" align="center">{{ item.chestRadiograph }}</uni-td>
					<uni-td align="center">{{ formatDate1(item.photoTime) }}</uni-td>
...
import { getById, dbName, tbScreenChestRadiograph, tbScreenSum } from '@/utils/sqlite';
...
export default {
	...
	methods: {
		...
		search() {
			...
			SynchronizeApi.getTableData4(this.queryParams).then((resp) => {
					...
					if (this.selectedIndexs.length == 0) {
                        ...
                        SynchronizeApi.removeNullProperties(item)
												...
												SynchronizeApi.selectTableIdByIndex(
													...
													item.photoTime = self.formatDate(item.photoTime);
													...
													if (res.length > 0) {
														...
														getById(dbName, tbScreenChestRadiograph, item.id).then(
															(res) => {
																if (res.length > 0) {
                    ...
                    SynchronizeApi.removeNullProperties(item)
										...
										SynchronizeApi.selectTableIdByIndex(
											...
											if (res.length > 0) {
												...
												delete updateData.id;
												delete updateData.name;
												SynchronizeApi.updateTable(tbScreenChestRadiograph, updateData, id);
											} else {
												getById(dbName, tbScreenChestRadiograph, item.id).then((res) => {
													if (res.length > 0) {
														SynchronizeApi.selectMaxId(tbScreenChestRadiograph).then(
															(resp) => {
																if (resp.length > 0) {
																	let maxId = resp[0].maxId;
																	let newUpdateData = {
														...
														delete addData.name;
														dbUtils.addTabItem(dbName, tbScreenChestRadiograph, addData);
													}
												});
											}
										});
