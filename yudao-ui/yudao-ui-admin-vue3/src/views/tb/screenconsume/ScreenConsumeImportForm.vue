<template>
  <Dialog v-model="dialogVisible" title="消耗管理导入" width="400">
    <el-upload
      ref="uploadRef"
...
import {ScreenConsumeApi} from "@/api/tb/screenconsume";


defineOptions({ name: 'ScreenConsumeImportForm' })
...
const uploadRef = ref()
const importUrl =
  import.meta.env.VITE_BASE_URL + import.meta.env.VITE_API_URL + '/tb/screen-consume/import-excel'
...
const importTemplate = async () => {
  const res = await ScreenConsumeApi.importUserTemplate()
  await download.excel(res, '消耗管理导入模版.xls')
  return message.success("成功下载消耗管理导入模板！")
}
